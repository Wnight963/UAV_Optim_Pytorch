...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            # landmark.state.p_pos = np.array([[0.89699076, -0.14581629], [-0.8154612, -0.24502182],
            #                                  [0.65862443, 0.23426809], [-0.3249325, 0.60295496],
            #                                  [0.62360916, -0.52245444], [0.29590076, 0.54571629],
            #                                  [-0.89699076, -0.74682629], [-0.89699076, 0.14581629],
            #                                  [0.84629571, -0.98582649], [-0.14699076, -0.75584629]])[i]
    ...
    def reward(self, agent, world):
        ...
        try:
            ...
            # reward = 10*np.log(p*np.min(world.Rate[np.nonzero(world.Rate)])+(1-p)*np.average(world.Rate[np.nonzero(world.Rate)]))
            # reward = 10 * np.log(np.min(world.Rate[np.nonzero(world.Rate)]))

            # 下面的方式寻找最小信息率的用户比较保险，避免当所有的rate均为零时，上一种方式会出错
            min_rate = None
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            other_comm.append(other.state.c)
        ...
        # return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + other_comm) # original code
        return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + assign + normalized_SNR)